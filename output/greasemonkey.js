...
function getHashOfUtterances(interactionModel){let utterances=interactionModel.utterances;let output={};if(utterances){for(let i=0;i<utterances.length;i++){let str=utterances[i];str=str.trim();[key,value]=str.match(/^(\S+)\s(.*)/).slice(1);key=key.trim();value=value.trim();if(!output[key]){output[key]=[]}output[key].push(value)}}return output}/*
     ...
     * 
     */
function getArrayOfUtterances(interactionModel){let utterances=interactionModel.utterances;let output=[];if(utterances){for(let i=0;i<utterances.length;i++){let str=utterances[i];str=str.trim();let value=str.substr(str.indexOf(" ")+1).trim();// Utterance
output.push(value)}}return output}/*
     * Get the utterances by intent with their slot type replaced with slot name
     *
     */
function getSlotReplacedHashOfUtterances(interactionModel){let intents=getHashOfIntents(interactionModel);let utterances=getHashOfUtterances(interactionModel);
//go through each one of the intents, check if they match in utterances based on the calculations above
for(let intentProp in intents){if(!intents.hasOwnProperty(intentProp)){continue}//property is from prototype, skip
let intent=intents[intentProp];let intentSlots=intent.slots;let intentUtterances=utterances[intentProp];if(intentUtterances){for(let i=0;i<intentUtterances.length;i++){let utterance=intentUtterances[i];let replacedUtterance=utterance;
//if we have intent slots, replace all the SlotNames with SlotTypes
if(intentSlots){for(let j=0;j<intentSlots.length;j++){let slot=intentSlots[j];replacedUtterance=replacedUtterance.replaceAll("{"+slot.name+"}","{"+slot.type+"}")}intentUtterances[i]=replacedUtterance}}}}return utterances}/*
     * Get the slot replaced sorted utterances without intent name
     *
     */
function getSlotReplacedArrayOfUtterances(interactionModel){let intents=getSlotReplacedHashOfUtterances(interactionModel);let output=[];for(let intentProp in intents){if(!intents.hasOwnProperty(intentProp)){continue}//property is from prototype, skip
let intent=intents[intentProp];for(let i=0;i<intent.length;i++){output.push(intent[i])}}return output}/*
...
function getHashOfSlots(interactionModel){let slots=interactionModel.slots;let output={};for(let i=0;i<slots.length;i++){let slot=slots[i];output[slot.name]=slot}return output}/*
     ...
     * true/false
     */
function skillHasAccountLinking(skillInformation){return skillInformation.metadata&&skillInformation.metadata.skillDefinition&&skillInformation.metadata.skillDefinition.accountLinkingInfo&&skillInformation.metadata.skillDefinition.accountLinkingInfo.supportsLinking}function getUtteranceValidCharactersRegex(locale){
//this can be extended to support other locales
switch(locale.toLowerCase()){case"de_de":return/[^\w'.,\-{}\sßäëïöü]/;default:return/[^\w'.,\-{}\s]/}}function getExamplePhraseValidCharactersRegex(locale){
//this can be extended to support other locales
...
tests.push({name:"Built In Intents Present",description:"Test to check whether Intents contains AMAZON.CancelIntent, AMAZON.HelpIntent, AMAZON.StopIntent"+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#providing-help'>DOCUMENTATION</a>"+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#stopping-and-canceling'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let intents=getHashOfIntents(interactionModel);if(!intents["AMAZON.CancelIntent"]){errors.push("AMAZON.CancelIntent is not present")}if(!intents["AMAZON.HelpIntent"]){errors.push("AMAZON.HelpIntent is not present")}if(!intents["AMAZON.StopIntent"]){errors.push("AMAZON.StopIntent is not present")}callback(test,errors,warnings)}});/*
     ...
     */
tests.push({name:"Utterances Reference Intents",description:"Test to check whether the sample utterances all refer to an existing Intent.",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let intents=getHashOfIntents(interactionModel);let utterances=getHashOfUtterances(interactionModel);for(let prop in utterances){if(!utterances.hasOwnProperty(prop)){continue}//property is from prototype, skip
if(!intents[prop]){errors.push("Intent '"+prop+"' not found")}}callback(test,errors,warnings)}});/*
     * Test to check whether all Intents have sample utterances. 
     * Warn: < 5 Sample Utterances per Intent
     */
tests.push({name:"Intents Have Utterances",description:"Test to check whether all Intents have sample utterances. Warn: < 5 Sample Utterances per Intent",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let intents=getHashOfIntents(interactionModel);let utterances=getHashOfUtterances(interactionModel);for(let prop in intents){if(!intents.hasOwnProperty(prop)){continue}//property is from prototype, skip
if(!utterances[prop]){if(prop.indexOf("AMAZON.")!==0){
//not a built-in intent
errors.push("Intent '"+prop+"' has no Sample Utterances")}}else if(utterances[prop].length<5){if(prop.indexOf("AMAZON.")!==0){
//not a built-in intent
warnings.push("Intent '"+prop+"' has < 5 Sample Utterances")}}}callback(test,errors,warnings)}});/*
     * Test to check whether all required Slots (as referenced in Intents) are defined
     */
tests.push({name:"All Intent Slots Are Defined",description:"Test to check whether all required Slots (as referenced in Intents) are defined",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let slots=getHashOfSlots(interactionModel);let intents=getHashOfIntents(interactionModel);for(let prop in intents){if(!intents.hasOwnProperty(prop)){continue}//property is from prototype, skip
let intent=intents[prop];let intentSlots=intent.slots;if(intentSlots){for(let i=0;i<intentSlots.length;i++){let slot=intentSlots[i];if(slot.type.indexOf("AMAZON.")===0){
//built in slot, ignore
continue}if(!slots[slot.type]){errors.push("Intent '"+prop+"' has undefined Slot Type '"+slot.type+"'")}}}}callback(test,errors,warnings)}});/*
     * Test to check whether all required Slots (as referenced in Utterances) are defined in Intents
     */
tests.push({name:"All Utterance Slots Defined In Intent",description:"Test to check whether all required Slots (as referenced in Utterances) are defined in Intents",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let regexp=/([^{]*?)\w(?=\})/gim;let intents=getHashOfIntents(interactionModel);let utterances=getHashOfUtterances(interactionModel);for(let prop in utterances){if(!utterances.hasOwnProperty(prop)){continue}//property is from prototype, skip
let intentUtterances=utterances[prop];let intent=intents[prop];
//if we have no utterances for this intent
if(!intentUtterances){continue}for(let x=0;x<intentUtterances.length;x++){let utterance=intentUtterances[x];
...
if(!matches){continue}for(let i=0;i<matches.length;i++){let match=matches[i];if(intent&&intent.slots){let found=false;for(let j=0;j<intent.slots.length;j++){let slot=intent.slots[j];if(slot.name===match){found=true;break}}if(!found){errors.push("Intent '"+prop+"' missing Slot '"+match+" in Utterance '"+utterance+"'")}}}}}callback(test,errors,warnings)}});/*
     * Test to check whether all Slots have at least one value.
     * Warn: <2 values
     */
tests.push({name:"All Slots Have Values",description:"Test to check whether all Slots have at least one value."+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#custom-slot-type-values'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let slots=getHashOfSlots(interactionModel);for(let prop in slots){if(!slots.hasOwnProperty(prop)){continue}//property is from prototype, skip
let slot=slots[prop];if(!slot.values||slot.values.constructor!==Array||slot.values.length<1){errors.push("Slot '"+slot.type+"' has no Values")}else if(slot.values.length<2){warnings.push("Slot '"+slot.type+"' has < 2 Values")}}callback(test,errors,warnings)}});/*
     * Test to check whether an individual Slot's Values have a variety of word lengths.
     * Warn: Delta < 2 words
     */
tests.push({name:"Slot Values Have Length Variety",description:"Test to check whether an individual Slot's Values have a variety of word lengths. Warn: Delta < 2 words"+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#custom-slot-type-values'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let slots=getHashOfSlots(interactionModel);for(let prop in slots){if(!slots.hasOwnProperty(prop)){continue}//property is from prototype, skip
let slot=slots[prop];let min=0;let max=0;if(slot.values&&slot.values.constructor===Array){for(let i=0;i<slot.values.length;i++){let length=slot.values[i].split(" ").length;min=Math.min(min,length);max=Math.max(max,length)}}if(max-min<2){warnings.push("Slot '"+prop+"' has word length delta < 2 words")}}callback(test,errors,warnings)}});/*
     * Test to check whether an individual Intent's Utterances have a variety of word lengths.
     * Warn: Delta < 5 words
     */
tests.push({name:"Intent Utterances Have Length Variety",description:"Test to check whether an individual Intent's Utterances have a variety of word lengths. Warn: Delta < 5 words"+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#variety-of-sample-utterances'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let utterances=getHashOfUtterances(interactionModel);for(let prop in utterances){if(!utterances.hasOwnProperty(prop)){continue}//property is from prototype, skip
let intentUtterances=utterances[prop];let min=0;let max=0;if(intentUtterances&&intentUtterances.constructor===Array){for(let i=0;i<intentUtterances.length;i++){let length=intentUtterances[i].split(" ").length;min=Math.min(min,length);max=Math.max(max,length)}}if(max-min<2){warnings.push("Intent '"+prop+"' has Utterance word length delta < 5 words")}}callback(test,errors,warnings)}});/*
     * Test to check whether a custom slot has only supported characters.
     */
tests.push({name:"Unsupported Characters In Slot",description:"Test to check whether a custom slot has only supported characters."+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#custom-slot-type-values'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let regexp=/[^a-zA-Z\d\s\.\'ßäëïöü]/gi;let slots=getHashOfSlots(interactionModel);for(let prop in slots){if(!slots.hasOwnProperty(prop)){continue}//property is from prototype, skip
let slot=slots[prop];if(slot.values&&slot.values.constructor===Array){for(let i=0;i<slot.values.length;i++){let value=slot.values[i];let matches=value.match(regexp);if(matches&&matches.length>0){warnings.push("Slot '"+slot.name+"' Value should be alphanumeric except for contractions, possessive nouns, and acronyms: '"+value+"'")}}}}callback(test,errors,warnings)}});/*
...
tests.push({name:"No Duplicate Slots In Utterances",description:"Test to check whether a particular Intent Slot is used more than once in an Utterance."+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#variety-of-sample-utterances'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let regexp=/([^{]*?)\w(?=\})/gim;for(let i=0;i<interactionModel.utterances.length;i++){let utterance=interactionModel.utterances[i];let matches=utterance.match(regexp);let cache={};if(matches){for(let j=0;j<matches.length;j++){let key=matches[j];if(cache[key]){errors.push("Duplicate Slot in Utterance: "+utterance);break}cache[key]=true}}}callback(test,errors,warnings)}});/*
     ...
     */
tests.push({name:"Utterance Starts With Connector Word - Reduces Accuracy",description:"Test to check whether a an utterance starts with one of the connector words.",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let utterances=getHashOfUtterances(interactionModel);let intents=getHashOfIntents(interactionModel);
//go through each one of the intents, check if they match in utterances based on the calculations above
for(let intentProp in intents){if(!intents.hasOwnProperty(intentProp)){continue}//property is from prototype, skip
let intent=intents[intentProp];let intentUtterances=utterances[intentProp];if(intentUtterances){for(let i=0;i<intentUtterances.length;i++){if(getConnectorWordRegex(locale).test(intentUtterances[i])){warnings.push("Utterance '"+intentUtterances[i]+"' starts with a connector word")}}}}callback(test,errors,warnings)}});/*
     ...
     */
tests.push({name:"Sample Utterances Are Unique",description:"Test to check whether all Sample Utterances are unique. By nature this test is very slow, since it intelligently tries all the permutations.",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let utterances=getHashOfUtterances(interactionModel);let intents=getHashOfIntents(interactionModel);
...
addUtteranceChecks(intentProp,"",replacedUtterance,utterancesToCheck,associationMap);for(let j=0;j<utterancesToCheck.length;j++){let utteranceToCheck=utterancesToCheck[j].utterance;utteranceToCheck=utteranceToCheck.replaceAll(replaceexp,"");utteranceToCheck=utteranceToCheck.toLowerCase();if(conflictMap[utteranceToCheck]){let conflict=conflictMap[utteranceToCheck];let error="Intent '"+intentProp+"' Utterance '"+utterance+"' has collision with Intent '"+conflict.intent+"' Utterance '"+conflict.utterance+"'";if(intentProp!==conflict.intent){if(!errorHash[error]){errorHash[error]=0}errorHash[error]=errorHash[error]+1}else{if(!warningHash[error]){warningHash[error]=0}warningHash[error]=warningHash[error]+1}continue}else{
//add the value before we start swapping out slots
conflictMap[utteranceToCheck]={intent:intentProp,utterance:utterance}}}}}}for(let error in errorHash){if(!errorHash.hasOwnProperty(error)){continue}//property is from prototype, skip
errors.push(error+" ("+errorHash[error]+")")}for(let warning in warningHash){if(!warningHash.hasOwnProperty(warning)){continue}//property is from prototype, skip
warnings.push(warning+" ("+warningHash[warning]+")")}callback(test,errors,warnings)}});function getAssociationMap(interactionModel){let associationMap={};for(let i=0;i<interactionModel.slots.length-1;i++){let slot1Values=interactionModel.slots[i].values;slot1Values.sort();for(let j=i+1;j<interactionModel.slots.length;j++){let matched=false;let slot2Values=interactionModel.slots[j].values;slot2Values.sort();let pointer1=0,pointer2=0;while(pointer1<slot1Values.length&&pointer2<slot2Values.length){let compareValue=slot1Values[pointer1].toLowerCase().localeCompare(slot2Values[pointer2].toLowerCase());if(compareValue===0){matched=true;break}else if(compareValue===-1){pointer1++}else if(compareValue===1){pointer2++}}if(matched){let key1=interactionModel.slots[i].type;let key2=interactionModel.slots[j].type;if(!associationMap[key1]){associationMap[key1]=[]}if(!associationMap[key2]){associationMap[key2]=[]}
//we had an overlapping value so we need to make sure we equate the two
associationMap[key1].push(key2);associationMap[key2].push(key1)}}}return associationMap}
//our recursive method that creates all required permutations of matched items created above
function addUtteranceChecks(intent,utteranceStart,utteranceEnd,utterancesToCheck,associationMap,matches){if(!matches){let regexp=/([^{]*?)\w(?=\})/gim;matches=utteranceEnd.match(regexp)}
//base case, if we have nothing else to replace, put it into our array
if(!matches||matches.length===0){utterancesToCheck.push({intent:intent,utterance:utteranceStart+utteranceEnd});
//console.log(utteranceStart + utteranceEnd);
return}let match=matches[0];let replacements=associationMap[match];let childMatches=matches.slice(1,matches.length);if(replacements){let location=utteranceEnd.indexOf("{"+match+"}");let front=utteranceStart+utteranceEnd.substring(0,location);let back=utteranceEnd.substring(location+match.length+2,utteranceEnd.length);
//if we have items to replace
for(let j=0;j<replacements.length;j++){let replacement=replacements[j];addUtteranceChecks(intent,front,"{"+replacement+"}"+back,utterancesToCheck,associationMap,childMatches)}}else{
...
addUtteranceChecks(intent,utteranceStart,utteranceEnd,utterancesToCheck,associationMap,childMatches)}}/*
     ...
     */
tests.push({name:"Slot Used In Intent",description:"Test to check whether all Slots are used in an Intent.",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let slots=getHashOfSlots(interactionModel);let intents=getHashOfIntents(interactionModel);for(let slotType in slots){if(!slots.hasOwnProperty(slotType)){continue}//property is from prototype, skip
let found=false;for(let intentProp in intents){if(!intents.hasOwnProperty(intentProp)){continue}//property is from prototype, skip
let intent=intents[intentProp];if(intent.slots){for(let i=0;i<intent.slots.length;i++){let slot=intent.slots[i];if(slotType===slot.type){found=true}}}if(found){break}}if(!found){errors.push("Slot '"+slotType+"' never used in Intents")}}callback(test,errors,warnings)}});/*
     * Test to check whether all Slots defined in the Intent Schema are used in at least one Sample Utterance.
     */
tests.push({name:"Intents Use All Slots",description:"Test to check whether all Slots defined in the Intent Schema are used in at least one Sample Utterance.",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let intents=getHashOfIntents(interactionModel);let utterances=getHashOfUtterances(interactionModel);for(let prop in intents){if(!intents.hasOwnProperty(prop)){continue}//property is from prototype, skip
let intent=intents[prop];let intentSlots=intent.slots;if(!intentSlots){
//no slots for this intent
continue}for(let i=0;i<intentSlots.length;i++){let slotName=intentSlots[i].name;let intentUtterances=utterances[prop];let found=false;if(intentUtterances){for(let j=0;j<intentUtterances.length;j++){let utterance=intentUtterances[j];if(utterance.indexOf("{"+slotName+"}")!==-1){found=true;break}}}if(!found){errors.push("Intent '"+prop+"' never uses Slot '"+slotName+"'")}}}callback(test,errors,warnings)}});/*
     ...
     */
tests.push({name:"Utterances Abbreviations Are Formatted For Voice e.g.: 'TV' > 'T. V.'",description:"Test to check whether abbreviations are spelled out, this will not catch all abbreviatons and for that reason this is just a warning to raise a red flag to review. Checks for 2 capital letters next to each other or a capital letter followed by a period not followed by a space"+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-voice-interface-and-user-experience-testing#writing-conventions-for-sample-utterances'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];let regex=/[A-Z]{2,}|[A-Z]{1,}\.\w{1,}/;let utterances=getHashOfUtterances(interactionModel);for(let prop in utterances){if(!utterances.hasOwnProperty(prop)){continue}//property is from prototype, skip
let intentUtterances=utterances[prop];for(let i=0;i<intentUtterances.length;i++){let utterance=intentUtterances[i];
//remove our {SLOT} in case they conflict
let testUtterance=utterance.replace(/\{\w+\}/gi,"");let matches=testUtterance.match(regex);if(matches&&matches.length>0){warnings.push("Utterance '"+utterance+"' abbreviations should be formatted for voice.")}}}callback(test,errors,warnings)}});/*
...
tests.push({name:"First Example Phrases has Wake Word and Invocation Name",description:"Test to check whether first Example Utterance/Recommended Phrase has both the wake word and invocation name."+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-functional-testing#review-and-test-example-phrases'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];if(!metadata){console.warn("Metadata does not exist.");callback(null);return}let invocationName=metadata.invocationName;if(invocationName&&metadata.examplePhrases){invocationName=normalizeUtterance(invocationName);let invokeRegex=new RegExp("("+invocationName+")[^\\w]|("+invocationName+")$","gi");let examplePhrase=metadata.examplePhrases[0];examplePhrase=normalizeUtterance(examplePhrase);if(!examplePhrase.match(getWakeWordRegex(locale))||getLastCharacterPositionOfInvocationName(examplePhrase,invocationName)===-1){errors.push("First example phrase: '"+examplePhrase+" ' does not contain both the wakeword and the invocation name")}}else if(!invocationName){errors.push("Invocation name does not exist")}callback(test,errors,warnings)}});/*
     ...
     */
tests.push({name:"Example Phrases In Sample Utterances - No Check Against Amazon.*",description:"Test to check whether Example Utterances/Recommended Phrases included in Sample Utterances."+"<a href='https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/alexa-skills-kit-functional-testing#review-and-test-example-phrases'>DOCUMENTATION</a>",run:function(test,metadata,interactionModel,locale,callback){let errors=[];let warnings=[];if(!metadata){console.warn("Metadata does not exist.");callback(null);return}let utterances=getSlotReplacedArrayOfUtterances(interactionModel);let slots=getHashOfSlots(interactionModel);utterances.sort();if(metadata.examplePhrases&&metadata.examplePhrases.length>0){let invocationName=metadata.invocationName.toLowerCase();for(let i=0;i<metadata.examplePhrases.length;i++){let examplePhrase=metadata.examplePhrases[i];if(!examplePhrase){continue}
